...
export function SwipePage({ category }: { category: string }) {
  ...
  console.log(wallets, "custom message : wallets from privy");
  const embeddedWallet = wallets.find(w => w.walletClientType === "phantom");


  const MONAD_CHAIN_ID = '0x279f'; // 10143 in hex

  async function ensureMonadNetwork(ethereumProvider: any) {
  ...
  const getClient = async () => {
    if (embeddedWallet == undefined) {
      throw new Error("embedded wallet is undefined");
    }
    const provider = await embeddedWallet.getEthereumProvider();
    await ensureMonadNetwork(provider); // <-- Add this line
